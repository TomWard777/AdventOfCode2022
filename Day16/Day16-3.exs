...
defmodule Day16 do
  def run() do
    ...
    pathref = get_path_reference(nodes, 200)

    IO.puts("Computing random traversals...")
    reps = 100000

    {_, score} = repeat(nodes, "AA", reps, pathref)
    score
  end

  def repeat(nodes, start, reps, pathref, best_path \\ [], best_score \\ 0) do
    {nodes1, score1} = random_traverse(nodes, pathref, [start])
    {_, score2} = random_traverse(nodes1, pathref, [start])

    best_score = cond do
      score1 + score2 > best_score -> score1 + score2
      true -> best_score
    end
  ...
  def random_traverse(nodes, pathref, path, time_remaining \\ 26, total_score \\ 0) do
    [current | _] = path

    nodes_with_flow = nodes
    |> Enum.filter(fn x -> x.flow > 0 end)
    |> Enum.filter(fn x -> Enum.count(pathref[[current, x.key]]) < time_remaining end)
